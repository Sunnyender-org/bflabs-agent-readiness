...
5. For every rendered or customer-facing surface, apply this rule exactly: `用户看见的字只对使用者负责。实现约束用来遵守，不要写进产品。没见过这次讨论的人读不懂的句子，就不该出现。字贴边、最后一行只剩一两个字，也算没对使用者负责。` Keep runtime contracts, status enums, routing mechanics, validation receipts, and rollback procedures in agent-only references, code, or tests unless a user needs them to complete the task.
