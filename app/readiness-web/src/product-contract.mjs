...
export function leaderboardSharePage(entry) {
  const labels = { pass: '通过', partial: '部分通过', fail: '未通过', unknown: '证据不足', blocked: '无法检查' };
  const axes = entry.axes.map((axis) => `<article class="score-card" data-status="${escapeHtml(axis.status)}"><div class="score-meta"><span>${escapeHtml(axis.label)}</span><span class="score-tag">${escapeHtml(labels[axis.status] || '待确认')}</span></div><p class="score-number">${axis.score == null ? 'N/A' : escapeHtml(axis.score)}${axis.score == null ? '<small>证据不足</small>' : '<small>/100</small>'}</p></article>`).join('');
  ...
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(entry.host)} · BFLabs Agent Readiness 榜单</title><meta name="description" content="${escapeHtml(entry.host)} 主动公开的可发现、可理解、可操作结果。"><link rel="canonical" href="${canonical}"><link rel="icon" href="/bf-mark.svg" type="image/svg+xml"><link rel="preload" href="/fonts/space-grotesk-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="/fonts/inter-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="/styles.css"></head><body><header class="site-header"><a class="brand" href="/"><img src="/bf-mark.svg" alt=""><span class="brand-copy"><strong>BF LABS</strong><small>Agent Readiness</small></span></a><nav class="site-nav"><a href="/leaderboard">公开榜单</a><a href="https://skills.bflabs.cn/catalog.html#geo">修复工具</a></nav></header><main class="app-shell leaderboard-shell"><header class="leaderboard-hero"><span>站点主动公开</span><h1>${escapeHtml(entry.host)}</h1><p>这是该域名主动公开的可发现、可理解、可操作结果，检查于 ${escapeHtml(displayDate)}。外部 AI 平台表现和业务结果需要单独验证。</p><a class="button button-primary" href="/">诊断你的网站</a></header><section class="score-grid" aria-label="网站准备度结果">${axes}</section><p class="leaderboard-note">结果指纹：${escapeHtml(entry.scan_fingerprint)}</p></main><footer class="site-footer"><span>BFLabs Agent Readiness</span><nav><a href="/leaderboard">返回榜单</a><a href="/privacy">隐私边界</a></nav></footer></body></html>`;
}
...
export function leaderboardIndexPage(board) {
  ...
  }).join('') : '<li class="leaderboard-empty">榜单还没有公开结果。诊断你的网站并选择公开，就能加入这里。</li>';
  const storageNote = board.storage === 'ready'
    ? '榜单只保存域名、三项得分、检查时间和结果指纹。'
    : '榜单暂未开放，本次诊断仍可正常使用。';
  return `<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>公开榜单 · BFLabs Agent Readiness</title><meta name="description" content="BFLabs Agent Readiness 公开榜单，只收录站点提交者主动公开的可发现、可理解、可操作结果。"><link rel="canonical" href="${PUBLIC_BASE_URL}/leaderboard"><link rel="icon" href="/bf-mark.svg" type="image/svg+xml"><link rel="preload" href="/fonts/space-grotesk-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="preload" href="/fonts/inter-latin.woff2" as="font" type="font/woff2" crossorigin><link rel="stylesheet" href="/styles.css"></head><body><header class="site-header"><a class="brand" href="/"><img src="/bf-mark.svg" alt=""><span class="brand-copy"><strong>BF LABS</strong><small>Agent Readiness</small></span></a><nav class="site-nav"><a href="/">开始诊断</a><a href="https://skills.bflabs.cn/catalog.html#geo">修复工具</a></nav></header><main class="app-shell leaderboard-shell"><header class="leaderboard-hero"><span>站点主动公开</span><h1>公开榜单</h1><p>只展示站点提交者主动公开的当前结果，不代表站点与 BFLabs 存在合作或认可关系。外部 AI 平台表现和业务结果不参与排名。</p><a class="button button-primary" href="/">诊断你的网站</a></header><section aria-labelledby="ranking-title"><div class="leaderboard-heading"><h2 id="ranking-title">网站准备度排名</h2><p>先比较通过的项目数量，再比较最低项和三项平均分。</p></div><ol class="leaderboard-list">${rows}</ol><p class="leaderboard-note">${escapeHtml(storageNote)}</p></section></main><footer class="site-footer"><span>BFLabs Agent Readiness</span><nav><a href="/privacy">隐私边界</a><a href="/terms">使用边界</a></nav></footer></body></html>`;
}
