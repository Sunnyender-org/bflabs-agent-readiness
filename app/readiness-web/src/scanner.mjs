...
export function buildAgentPrompt({ origin, fingerprint, axes, findings, evidenceGaps, routes, journey = null }) {
  ...
  return [
    ...
    `- 根 Skill 的 Agent 约定与配套文件清单：${PUBLIC_SKILL_BASE}/skills/bflabs-agent-readiness/manifest.json；行动前先读取 ${PUBLIC_SKILL_BASE}/skills/bflabs-agent-readiness/references/root-agent-contract.md`,
