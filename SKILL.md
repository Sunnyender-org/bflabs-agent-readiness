...
Agent 请读取本文件原文，不要用搜索摘要代替说明；按原文中的版本使用配套文件。只读检查网站时，也先读取[诊断规则](https://readiness.bflabs.cn/skills/geo-optimize/references/readiness-rules.md)，再给结论。读取不到原文时如实说明，不猜版本或规则。
