...
本批改完、某一页已经上线，都不等于首轮建设完成，也不等于效果闭环。首轮建设要看重要问题是不是都有对应页面；多题可以共用同一页。效果闭环要看同一批问题的复测，以及你提供的业务窗口。这三件事分开看，不会合成一个分数。

1. 写下必须正确的事实，以及你希望 AI 能答对的问题。
2. 改网站之前，先收集现在的 AI 回答。
3. 对照事实，找出答错的问题和对应页面。先看首页、一张关键一级页和一张关键二级页；这只是起点，不是整站已经建完。
...
8. 交付一份可离线打开的前后对照页：本批修改、首轮建设、效果复盘分开看，原图和原文一起保留。写清变化和下一步。

中途停下也没关系。下次从还没做完的那一步继续。已经公开上线的改动，不要重做。问整站有没有做完时，先看还有哪些重要问题没有对应页面。
...
从六类核心问题开始：品牌身份、寻找官网、读懂页面、进入候选、场景选型、开始使用。再补价格、风险等业务问题，以及客户真实问过的问题。Agent 根据网站填写题目；新想到的问题留给下一轮，不改已经采样的题目。对照看板会说明每题测什么、怎么判断。
...
整套方法都免费。包括完整一轮怎么做、检查规则、模板和报告格式。免费方法本身已经完整，不是预览。

你自己准备 Agent、网站权限、平台账号或手头的回答记录，以及你自己的业务数据。

BFLabs 的服务，是由 BFLabs 来做这些事：改站实施、反复抽样、持续维护，以及后续复查。不要把市场上架同步说成已经完成。
