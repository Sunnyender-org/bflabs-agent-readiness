...
Before reporting a missing or broken fact from extracted text, compare the relevant raw HTML or public response with the rendered page when available. Text extraction can omit nested or aria-hidden content; distinguish an extraction/accessibility gap from an actually empty value. Report only the failure you verified. Absence from a sitemap is not proof of absence from a search index, and machine endpoints need not all appear in a page sitemap.
