#!/usr/bin/env python3
"""Validate the public geo-measure Skill package."""
...
with (ROOT / "templates/observations.csv").open("r", encoding="utf-8", newline="") as handle:
    if not list(csv.DictReader(handle)):
        fail("CSV template needs one example row")
...
for path in ROOT.rglob("*"):
    if not path.is_file() or path.suffix not in {".md", ".json", ".yaml", ".yml", ".py", ".csv"}:
        continue
...
print("PASS: geo-measure package ({} positive routes, {} negative routes)".format(positive, negative))
