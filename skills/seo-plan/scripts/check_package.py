#!/usr/bin/env python3
"""Validate the public seo-plan Skill package."""
...
REQUIRED = ["SKILL.md", "agents/openai.yaml", "references/technical-scope.md", "templates/seo-plan-brief.json", "examples/migration-brief.json", "evals/trigger_cases.json"]
...
front = re.match(r"^---\n(.*?)\n---\n", skill, re.S)
if not front or "name: seo-plan" not in front.group(1) or "Not for" not in front.group(1):
    fail("invalid SKILL.md frontmatter")
for relative in ["references/technical-scope.md", "templates/seo-plan-brief.json"]:
    if relative not in skill:
        fail("SKILL.md does not route to " + relative)
for relative in ["templates/seo-plan-brief.json", "examples/migration-brief.json", "evals/trigger_cases.json"]:
...
print("PASS: seo-plan package ({} positive routes, {} negative routes)".format(positive, negative))
